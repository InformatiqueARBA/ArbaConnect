...
class CoutingPageXLSXService
{
    ...
    public function generateCountingXLSXStock(array $articlesByLocation, $location, string $filePath, $inventoryNumber, $cptMax): PdfWriter
    {
        ...
        foreach ($articlesByLocation as $index => $inventoryArticle) {
            ...
            if (($index + 1) % $rowsPerPage == 0) {
                ...
                $sheet->setCellValue('A' . $rowIndex, 'p.' . $currentPage . '/' . $totalPages . ' de l\'allée');
                $sheet->setCellValue('H' . $rowIndex, 'p.' . $this->cpt . '/' . $cptMax . ' de l\'inventaire');
    ...
    public function countingMax(array $articlesByLocation, $location, string $filePath, $inventoryNumber)
    {
        $spreadsheet = new Spreadsheet();
        $sheet = $spreadsheet->getActiveSheet();
        $sheet->getPageSetup()->setOrientation(PageSetup::ORIENTATION_LANDSCAPE);
        ...
        foreach ($articlesByLocation as $index => $inventoryArticle) {
            ...
            if (($index + 1) % $rowsPerPage == 0) {
                ...
                $sheet->mergeCells('A' . $rowIndex . ':G' . $rowIndex);
                $sheet->setCellValue('A' . $rowIndex, 'p.' . $currentPage . '/' . $totalPages . ' de l\'allée');
                $sheet->setCellValue('H' . $rowIndex, 'p.' . $this->cpt2 . ' de l\'inventaire');
                $sheet->getStyle('A' . $rowIndex)->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
                $sheet->getStyle('H' . $rowIndex)->getAlignment()->setHorizontal(Alignment::HORIZONTAL_RIGHT);
                $currentPage++;
    ...
    public function countingMaxLot(array $articlesByLocation, $location, string $filePath, $inventoryNumber)
    {
        $spreadsheet = new Spreadsheet();
        $sheet = $spreadsheet->getActiveSheet();
        $sheet->getPageSetup()->setOrientation(PageSetup::ORIENTATION_LANDSCAPE);
